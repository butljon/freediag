...
static int diag_l2_proto_kw1281_recv(struct diag_l2_conn *d_l2_conn, int timeout, void (*callback)(void *handle,
		struct diag_msg *msg), void *handle);

static int diag_l2_proto_kw1281_send(struct diag_l2_conn *d_l2_conn, struct diag_msg *msg);
...
static int diag_l2_proto_kw1281_recv_msg(struct diag_l2_conn *d_l2_conn, int timeout __attribute__((unused))) {
	...
	rv = diag_l2_proto_kw1281_recv_byte(d_l2_conn, timeout, &databyte, 0);
	if(rv < 0) {
 	    fprintf(stderr, FLFMT "Could not receive sequence byte\n", FL);
	    return rv;
   	}
	...
	if(rv < 0) {
 	    fprintf(stderr, FLFMT "Could not receive the type byte\n", FL);
	    return rv;
   	}
	...
	if(rv < 0) {
 	    fprintf(stderr, FLFMT "Could not receive end of frame byte\n", FL);
	    return rv;
   	}

	if(databyte != DIAG_VAG_END_FRAME) {
 	    fprintf(stderr, FLFMT "Should have been end of frame byte, instead we got: %x\n", FL, databyte);
	    return -1;
   	}
...
static int diag_l2_proto_kw1281_send_byte(struct diag_l2_conn *d_l2_conn, databyte_type databyte, int end) {
    ...
    if(end)
	{
	    ...
	    if(rv < 0) {
	        fprintf(stderr, FLFMT "Failed to receive back the compliment of %x (i.e. %x)\n", FL, databyte, (~databyte & 0xff));
	      return rv;
	    }
...
static struct diag_msg *diag_l2_proto_kw1281_request(struct diag_l2_conn *d_l2_conn, struct diag_msg *msg, int *errval) {
 ...
 * VAG Stopcomms
 */
int diag_l2_proto_kw1281_stopcomms(struct diag_l2_conn* d_l2_conn __attribute__((unused))) {
