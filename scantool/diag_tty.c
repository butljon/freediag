...
int
diag_tty_check_set(int fd, struct termios *termios_p) {
  

	/* according to manpage tcsetattr returns success even if only one parameter setting was succesful:
	 * therefore double check!
	 */

	struct termios termios_tmp;

	
	if (tcgetattr(fd, &termios_tmp) < 0) {
		fprintf(stderr, 
			FLFMT "Can't obtain attributes, tcgetattr returned \"%s\".\n", FL, strerror(errno));
		return diag_iseterr(DIAG_ERR_GENERAL);	  
	}
	
	if(memcmp(&termios_tmp.c_iflag, &termios_p->c_iflag , sizeof(tcflag_t))) {
		fprintf(stderr, 
			FLFMT "c_iflag not set correctly\n", FL);
		return diag_iseterr(DIAG_ERR_GENERAL);	  
	}
	if(memcmp(&termios_tmp.c_oflag, &termios_p->c_oflag , sizeof(tcflag_t))) {
		fprintf(stderr, 
			FLFMT "c_oflag not set correctly\n", FL);
		return diag_iseterr(DIAG_ERR_GENERAL);	  
	}
/* 	if(memcmp(&termios_tmp.c_cflag, &termios_p->c_cflag , sizeof(tcflag_t))) {
 		fprintf(stderr, 
 			FLFMT "c_cflag not set correctly\n", FL);
 		return diag_iseterr(DIAG_ERR_GENERAL);	  
	}
*/	if(memcmp(&termios_tmp.c_lflag, &termios_p->c_lflag , sizeof(tcflag_t))) {
		fprintf(stderr, 
			FLFMT "c_lflag not set correctly\n", FL);
		return diag_iseterr(DIAG_ERR_GENERAL);	  
	}
	if(memcmp(&termios_tmp.c_cc, &termios_p->c_cc , sizeof(cc_t)*NCCS)) {
		fprintf(stderr, 
			FLFMT "c_cc not set correctly\n", FL);
		return diag_iseterr(DIAG_ERR_GENERAL);	  
	}
 ...
 */
int
new_diag_tty_setup(struct diag_l0_device *dl0d,
...
/*	switch (pset->speed) {
		...
		case 9600:
			...
			break;
		case 10400:
			fprintf(stderr, FLFMT "%d not supported as it is not standard baud rate, see termios.h"
			    "to make this work custom baud rate setting must be fixed, see ftdi_sio.c for how it should be done \n", FL, pset->speed);
			break;
		default:
			fprintf(stderr, FLFMT "bad speed setting used (%d)\n", FL, pset->speed);
			return diag_iseterr(DIAG_ERR_GENERAL);
	}
	if (cfsetispeed(&dt->dt_tinfo, speed) < 0) {
		fprintf(stderr,
			FLFMT "cfsetispeed failed: %s\n", FL, strerror(errno));
		return diag_iseterr(DIAG_ERR_GENERAL);
	}
	if (cfsetospeed(&dt->dt_tinfo, speed) < 0) {
		fprintf(stderr,
...
*/
	...
	switch (pset->speed) {
		case 1200:
			dt->dt_tinfo.c_cflag |= B1200;
			break;
		case 2400:
			dt->dt_tinfo.c_cflag |= B2400;
			break;
		case 4800:
			dt->dt_tinfo.c_cflag |= B4800;
			break;
		case 9600:
			dt->dt_tinfo.c_cflag |= B9600;
			break;
		case 10400:
			fprintf(stderr, FLFMT "%d not supported as it is not standard baud rate, see termios.h"
			    "to make this work custom baud rate setting must be fixed, see ftdi_sio.c for how it should be done \n", FL, pset->speed);
	...
	if(rv = diag_tty_check_set(fd, &dt->dt_tinfo))
	    return rv;
...
diag_tty_read(struct diag_l0_device *dl0d, void *buf, size_t count, int timeout)
{
	...
	rv = ioctl(fd, RTC_IRQP_READ, &data);
        if (rv < 0) {
		fprintf(stderr, 
			FLFMT "diag_tty_read: RTC_IRQP_READ failed %s\n", FL, strerror(errno));
		return diag_iseterr(DIAG_ERR_GENERAL);
	}
	...
	if(ioctl(fd, RTC_PIE_ON, 0) < 0) {
		fprintf(stderr, 
			FLFMT "diag_tty_read: RTC_PIE_ON failed %s\n", FL, strerror(errno));
		return diag_iseterr(DIAG_ERR_GENERAL);
	}
	...
	if(ioctl(fd, RTC_PIE_OFF, 0) < 0) {
		fprintf(stderr, 
			FLFMT "diag_tty_read: RTC_PIE_OFF failed %s\n", FL, strerror(errno));
		return diag_iseterr(DIAG_ERR_GENERAL);
	}
	close(fd);
...
//        if (ioctl(dl0d->fd, TIOCSBRK, 0) < 0) {
        retries = 9;
        rv = ioctl(dl0d->fd, TIOCSBRK, 0);
	while (rv == -1 && errno == EINTR && (retries--))
	  rv = ioctl(dl0d->fd, TIOCSBRK, 0);
        if (rv < 0) {
		fprintf(stderr, 
